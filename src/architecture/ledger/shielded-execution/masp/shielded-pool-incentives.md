...
The total incetives that are paid out, $I_L$, is minted each epoch based on the current parameters and are calculated according to the [inflation model](./specs/blob/master/src/architecture/economics/inflation-system.md). This total is then distributed evenly among recipients. 
