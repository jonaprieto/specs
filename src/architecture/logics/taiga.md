...
A valid **transaction** is defined as a *balanced* set of valid **partial transactions**. A partial transaction is valid when:
...
|$E_M$|Main curve|$\mathbb{F}_p$|$\mathbb{F}_q$|Action and VP circuits| [Vesta](https://github.com/zcash/pasta#pallasvesta-supporting-evidence)|
...
## 2. Notes
*Note* is an immutable particle of an application state. The state is updated by destroying or creating notes.
...
|Variable|Type/size|Description|
|-|-|-|
|`cm_app_vk`|| Contains the application's main VP verifier key. Used to identify the application the note belongs to. As the verifier key itself is large, notes only store a commitment to it.|
